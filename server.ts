...
// Migration: Add age, gender, image if they don't exist
try {
...
async function startServer() {
  ...
  const createTransporter = async (req?: any) => {
    ...
    if (!refreshToken) {
      console.warn("⚠️ No Google Refresh Token found. Email notifications are currently disabled.");
      console.warn(`👉 Please authorize the application at: ${getAppUrl(req)}/api/admin/google-auth`);
      return null;
    }
    ...
    try {
      ...
      const transporter = nodemailer.createTransport({
        ...
        auth: {
          type: "OAuth2",
          user: process.env.GOOGLE_USER || "forworking4343@gmail.com",
  ...
  app.get("/api/me", authenticateToken, (req: any, res) => {
    const user = db.prepare("SELECT id, name, email, phone, age, gender, image FROM users WHERE id = ?").get(req.user.id) as any;
    if (!user) return res.status(404).json({ error: "User not found" });
    res.json(user);
  });

  app.put("/api/me", authenticateToken, (req: any, res) => {
    let { name, phone, age, gender, image } = req.body;
    const userId = req.user.id;

    // Sanitize inputs
    const sanitizedAge = age === "" || age === null ? null : Number(age);
    const sanitizedGender = gender === "" ? null : gender;
    const sanitizedImage = image === "" ? null : image;

    try {
      const stmt = db.prepare("UPDATE users SET name = ?, phone = ?, age = ?, gender = ?, image = ? WHERE id = ?");
      stmt.run(name, phone, sanitizedAge, sanitizedGender, sanitizedImage, userId);

      const updatedUser = db.prepare("SELECT id, name, email, phone, age, gender, image FROM users WHERE id = ?").get(userId);
